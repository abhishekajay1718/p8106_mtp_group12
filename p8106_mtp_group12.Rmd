...
model_lm = lm(cnt~ season +  yr + mnth + holiday + weekday + workingday + weathersit + temp + atemp + hum + windspeed, data = day)
summary(model_lm)

ctrl1 <-trainControl(method = "cv", number = 10)
set.seed(1)
lmFit <-train(cnt~ season +  yr + mnth + holiday + weekday + workingday + weathersit + temp + atemp + hum + windspeed,data = day,method = "lm",trControl = ctrl1)
lmFit
```
...
featurePlot(x, y, plot = "scatter", labels =c("","Y"),type =c("p"), layout =c(10, 3))
```
