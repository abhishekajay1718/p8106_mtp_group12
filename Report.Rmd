...
The bike sharing dataset obtained from the UCI Machine Learning Repository represents bike rental data from the Capital Bikeshare system in Washington, D.C.. We perform the following analysis of a bike rental service in Washington D.C. to forecast the amount of bikes being rented using data from 2011 and 2012. The data set, as described below has various variables such as the weather, humidity, temperature, whether it's a holiday or not, etcetera. We plan to 
...
Since rescaling the outcome variable will change both estimates and MSE, we chose to keep the original scale and its correlation to the predictors. Furthermore, we can still compare the MSE of other models since they should all be very large. 
